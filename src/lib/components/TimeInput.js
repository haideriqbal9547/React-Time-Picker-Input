...
function TimeInput(props) {
  ...
  const [isMobile, setIsMobile] = useState(isMobileNeeded);
  ...
  const setHour = useCallback(
    (newHour, other = {}) =>
      setTime((t) => ({ ...t, hour: newHour, ...other })),
    [setTime]
  );
  const setMinutes = useCallback(
    (newMinute, other = {}) =>
      setTime((t) => ({ ...t, minute: newMinute, ...other })),
    [setTime]
  );
  const setAmPM = useCallback(
    (newAmPm, other = {}) =>
      setTime((t) => ({ ...t, amPm: newAmPm, ...other })),
    [setTime]
  );

  const hourRange = useMemo(
    () => (hour12Format ? { start: 1, end: 12 } : { start: 0, end: 23 }),
    [hour12Format]
  );
  ...
  const updateTouchDevice = () => setIsMobile(isMobileNeeded);
  ...
  return (
    <div className='react-time-input-picker-wrapper'>
      <div
        className={`react-time-input-picker ${disabled ? "is-disabled" : ""}`}
      >
        {isMobile ? (
...
format12Data.forEach((hour, index) => {
  ...
  if (hourInNumber === 0) {
    ...
    format12Data[index] = `${doubleChar(
      hourInNumber - 12
    )} : ${doubleCharMinutes}  PM`;
  }
});
...
const Options = memo((props) => {
  ...
  const [fullTimeDropdownVisibility, setFullTimeDropdownVisibility] =
    useState(false);
